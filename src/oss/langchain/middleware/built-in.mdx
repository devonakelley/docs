...
<Note>
    `ModelFallbackMiddleware` handles **reactive** fallback — it tries the next model only when the current one fails. For **proactive**, outcome-aware routing that learns from production results and continuously optimizes which model, tools, and parameters to use, see the [Kalibr integration](/oss/integrations/chat/kalibr).
</Note>
